...
add_dynamic_cmd_param "-attr \"START_PROGRAM='/sbin/mount.dbfs /mnt/dbfs'"
add_dynamic_cmd_param "    ,STOP_PROGRAM='/bin/fusermount -u /mnt/dbfs'"
add_dynamic_cmd_param "    ,CLEAN_PROGRAM='/bin/fusermount -u -z /mnt/dbfs'"
add_dynamic_cmd_param "    ,CHECK_PROGRAMS='/usr/bin/ls /mnt/dbfs'"
add_dynamic_cmd_param "    ,HOSTING_MEMBERS=$(hostname -s)"
add_dynamic_cmd_param "    ,PLACEMENT=restricted"
add_dynamic_cmd_param "    ,START_DEPENDENCIES=hard(ora.${db_name}.${service_name}.svc)"
add_dynamic_cmd_param "    ,STOP_DEPENDENCIES=hard(ora.${db_name}.${service_name}.svc)\""
exec_dynamic_cmd -confirm "crsctl add resource $resource_name"
