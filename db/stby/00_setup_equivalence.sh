...
Le script doit être exécuter depuis le host, il n'y aura pas de mot de passe
demandé.
"
...
function test_join_server
{
	typeset	-r	server_name="$1"

	info -n "$server_name joignable : "
...
line_separator
typeset -i	nr_join_failed=0
test_join_server $server1
[ $? -ne 0 ] && nr_join_failed=nr_join_failed+1

test_join_server $server2
[ $? -ne 0 ] && nr_join_failed=nr_join_failed+1

[ $nr_join_failed -ne 0 ] && exit 1
LN
