...
as
	dg_data varchar2(100) := '''DATA.FG'||inst_no||'''';
	dg_FRA varchar2(100) := '''FRA.FG'||inst_no||'''';

	stmt varchar2(1000) := 'alter system set asm_preferred_read_failure_groups='||dg_data||','||dg_fra||' scope=both sid='''||inst_name||'''';
begin
	p( '>'||stmt );
	execute immediate stmt;
end apply_asm_pref;
...
procedure main
as
	instance1 varchar2(30) := get_instance_name( 1 );
	instance2 varchar2(30) := get_instance_name( 2 );
begin
	p( '1 === '||instance1 );
